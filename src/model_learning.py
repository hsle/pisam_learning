...
from ..pddl.conditions import Conjunction, Literal
...
def get_precondition_effect_po(model, trajectories):
    for action in model.schemata:
        all_literals = generate_all_possible_literals(model.predicates, action.parameters)
        effects = []
        ...
        for trajectory in trajectories:
            for index in range(trajectory.length - 1):
                ...
                all_literals = [literal for literal in all_literals if (literal.rename_variables(param_binding)
                                                                        in pre_state.literals and literal.rename_variables(
                            param_binding)
                                                                        in post_state.literals) or (
                                        len(
                                            literal.args) == 0 and literal in pre_state.literals and literal in post_state.literals)]
