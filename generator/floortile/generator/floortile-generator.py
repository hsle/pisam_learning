...
def get_init():
   ...
   for row in range(num_rows):
      for column in range (num_columns):
         str_init += "   (up " + get_tile(row+1,column+1) + " " + get_tile(row,column+1) + ")\n"
   for row in range(num_rows):
      for column in range (num_columns):
         str_init += "   (down " + get_tile(row,column+1) + " " + get_tile(row+1,column+1) + ")\n"
   for row in range(num_rows+1):
      for column in range (num_columns-1):
         str_init += "   (right " + get_tile(row,column+2) + " " + get_tile(row,column+1) + ")\n"
   for row in range(num_rows+1):
      for column in range (num_columns-1):
         str_init += "   (left " + get_tile(row,column+1) + " " + get_tile(row,column+2) + ")\n"
...
def get_goals():
   ...
   for row in range(num_rows):
      for column in range (num_columns):
         if ((row+column) % 2 == 0):
            str_goals += "    (painted " + get_tile(row+1,column+1) + " white)\n"
         else:
            str_goals += "    (painted " + get_tile(row+1,column+1) + " black)\n"
   return str_goals + ")"
